...
l'attesa lo stato mostra **RICONNESSIONE…**, poi torna **ATTIVO**. **Solo al primo avvio**
della sessione i messaggi accumulati mentre il bridge era spento vengono **scartati**
(`drop_pending_updates`, così non si parte processando segnali vecchi). Sulle
**riconnessioni** successive, invece, il backlog **non** viene buttato: un segnale arrivato
durante un blip di rete di pochi secondi viene **recuperato** (con una riga di log
«🔄 Riconnesso…»), non perso per sempre. La protezione anti-arretrati resta comunque attiva
