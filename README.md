...
> percorso CSV restano quelli vecchi sarebbe fuorviante in modo pericoloso.
...
| `max_active_signals` | `2` | intero ≥ 1 (`0` = illimitato) | **Tetto di righe/scommesse attive** simultanee nelle modalità multi-riga (#136 p5): un nuovo segnale oltre il tetto viene **bloccato** (ritentabile quando una riga scade/è confermata). Default basso (2). Ininfluente in `OVERWRITE_LAST` (sempre 1 riga). La GUI mostra un indicatore **"Righe attive: N/M"**. Uno **`0` scritto apposta** vale «nessun tetto»; un valore **malformato** (testo, negativo, non intero, `NaN`/`inf`, o un numero troppo grande da rappresentare) ricade sul **default 2** — *non* su «illimitato, » come faceva prima della #194 PR-E: un tetto anti-overbetting non deve potersi spegnere da solo per un errore di battitura. |
...
| `confirmation_timeout` | `120` | secondi | **In `QUEUE_UNTIL_CONFIRMED`**: per quanti secondi un segnale resta in attesa della conferma XTrader prima di scadere (timeout per-segnale della coda). Nelle altre modalità coda non si applica: vale `clear_delay`. Deve essere un intero **> 0 e ≤ 86400** (24 h, AC-M7 audit #114 — stesso tetto anti-«segnale immortale» di `clear_delay`); un valore oltre tetto scritto a mano in `config.json` viene comunque **clampato a 24 h** dal runtime. |
...
| `auto_start_listener` | `false` | `true`/`false` | Se `true`, all'apertura l'app **avvia da sola** il listener — ma solo se token e **almeno una chat ammessa** sono configurati: chat singola, override `parser_by_chat`, **lista multi-parser `parser_list_by_chat`** (P3-3 #76) o una sorgente multi-chat ATTIVA (sorgenti tutte disattivate = niente avvio automatico); in **modalità reale** chiede conferma prima di partire. Un valore malformato (typo) vale come `false` anche al **salvataggio** (fail-closed). Default `false`: il bridge parte solo con **AVVIA**. Attivabile dalla tab *Sicurezza*. |
...
4. **Anti-duplicato** — lo stesso messaggio ravvicinato non viene riscritto. Lo
   ...
   **sostituisce** la precedente, quindi una seconda riga non può esistere.
   ...
   scritture equivalenti dell'handicap (`0` = `0.0` = `+0`, `0.50` = `0.5`). Dal medesimo audit
...
l'attesa lo stato mostra **RICONNESSIONE…**, poi torna **ATTIVO**. Le attese crescenti
**ripartono da capo a ogni AVVIA**: se una sessione precedente era arrivata al massimo,
premere AVVIA riparte da 2s, non da 60s — altrimenti, dopo aver sistemato la rete e
riavviato, il primo intoppo costerebbe un minuto di attesa senza che nulla lo spieghi. Se Telegram risponde con
