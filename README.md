...
> 🖼️ **Una sola eccezione dichiarata: `pillow`.** È installato con un `pip install "pillow==12.3.0"`
> dentro il job `unit` di `pr-checks.yml`, **non** in `requirements-dev.txt`. Serve a un test solo —
...
> test in `tests/unit/test_website_api_key_anthropic.py` pretende che la riga resti e resti pinnata.
