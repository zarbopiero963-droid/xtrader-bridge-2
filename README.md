...
| `app_language` | `""` | `""`/`IT`/`EN`/`ES` | **Lingua dell'applicazione (#343)**: scelta col **selettore al primo avvio** (🌐 IT/EN/ES). `""` = mai scelta → il selettore ricompare al prossimo avvio (nel frattempo il bridge resta nel comportamento storico IT). Alla scelta viene **allineata anche `csv_language`**. Valore malformato → `""` (fail-closed: il selettore riappare, mai un default spacciato per scelta). La UI localizzata arriva con un prossimo slice #343. |
