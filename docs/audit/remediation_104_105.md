...
| #105-P2 confirm CSV retry | mancava test: conferma → `write_rows` fallisce → coda già svuotata → retry riscrive → niente riscrittura dopo STOP | 🔧 test in `test_confirmation_flow.py` (questa PR; comportamento già corretto) |
