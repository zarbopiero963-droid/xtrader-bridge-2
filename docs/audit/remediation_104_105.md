...
| C5 | `load_config` senza migrazione/schema (tipi noti coerciti via `_migrate`) | `config_store.py` | `fix/audit-104-c5-c7` | 🔧 |
| C6 | `should_reconnect` classifica per nome classe sull'MRO | `reconnect_policy.py` | — | ⬜ |
| C7 | `save_config` ritorna shallow-copy con nested condivisi (ora `deepcopy`) | `config_store.py` | `fix/audit-104-c5-c7` | 🔧 |
