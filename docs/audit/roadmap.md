...
resta il **residuo LOG**: in `xtrader_bridge/app.py` le ~105 righe `self._log(...)` (messaggi
diagnostici) non passano ancora da i18n `tr()` e restano in italiano. Perciò la Issue #3
**resta aperta** finché anche i log non sono localizzati (slice successive, raggruppate per
area — decisione owner: banner prima, log a slice). *(Nota: una versione precedente scriveva
erroneamente «#3 CHIUSA» — era riferito alla sola slice 5, non all'epica.)*
...
di `BANNER_TEXT`/`COLLAUDO_BANNER_TEXT` fa fallire il catalogo (mai traduzioni orfane). Suite
**2414 passed, 11 skipped**. **CORE change** (`app.py`, `i18n.py`) → ri-sincronizzare nel cloud.
Docs: README + `design_handoff.md` (§ localizzazione: banner spostati da «restano IT» a
«localizzati»). Design handoff = **PASS** (aspetto UI di sicurezza cambiato: aggiornato).

**Ancora aperto (per chiudere #3):** localizzazione dei **~105 log** `self._log(...)` di
`app.py`, in slice raggruppate per area.
