...
| `merge-simulation-hard.yml` | manuale · schedulata (notte) | Windows: merge + suite completa + `safety`/`integration` + build EXE + controllo file vietati. Installa **`--require-hashes -r requirements-build.lock`** (CI-1 #114): builda un artefatto distribuibile, quindi serve il lock hashato — non `requirements-dev.txt` + `pyinstaller httpx` senza vincoli, com'era prima |
| `forbidden-files.yml` | PR · push main · manuale | blocca `.env`/`config.json`/`*.exe`/`*.zip`/`*.log`, gli stati del License Manager (`licenses.jsonl`, `revoked.jsonl`, backup col seed) e i CSV (eccetto `data/dizionario_xtrader.csv`); poi lo scan dei contenuti. Dalla PR-D (#194) non contiene più pattern propri: invoca `tools/forbidden_paths.py` + `tools/secret_scan.py`, che leggono la **fonte unica** `tools/secret_policy.py` |
| `build.yaml` | tag `v*` · manuale | job `build` = EXE Windows + artifact; job `build-linux` (#36) = app **Linux** = binario PyInstaller onefile impacchettato in **AppImage** (appimagetool pinnato+sha256) + artifact (`ubuntu-latest`, additivo, build Windows invariata); install dal **lock Linux con hash** se presente, altrimenti legacy; **solo** su tag `v*` (release) o `workflow_dispatch`. AppImage #36 PR-B ✅ |
