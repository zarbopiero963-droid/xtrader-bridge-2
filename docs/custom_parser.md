...
- nel pannello **🗺️ Mapping → ⚽ Calcio** i profili sono in una **colonna sempre visibile**
  ...
  quanti parser lo usano, e **«· N righe»**; l'elenco è **scorrevole** e **un click** apre il
  profilo (e **auto-salva** quello che stavi modificando) — da tastiera, Tab per raggiungerlo e
  **Invio/Spazio** per aprirlo. In coda compaiono i profili **⚠ «solo riferiti»** — richiesti da
  un parser ma **inesistenti** nel dizionario: sono righe di **avviso non cliccabili** (il profilo
  non esiste, non c'è nulla da aprire). I messaggi intercettati **da quel parser** producono
  `MAPPING_MISSING` e vengono scartati; gli altri parser della stessa chat non sono toccati. Vanno
  ricreati o tolti dalla spunta nel parser;
...
- **lingua della fonte (`source_language`, #3 slice 5a — FOUNDATION)**: il parser può
  ...
  Betting Toolkit). Dalla **slice 5b (wiring)** la pipeline calcola la `source_language`
  effettiva (override per-parser + globale) e la passa al filtro-lingua del dizionario nomi,
  **identica su live e anteprima** (invariante di parità): impostare la lingua-fonte **filtra
  davvero** il matching. Vuoto ovunque = comportamento storico (nessun filtro). Valore malformato
  → vuoto (fail-closed, retro-compatibile con i parser salvati prima del campo).
...
- **per-exchange Betfair (#3 slice 5d — completa la slice 5 «dizionario per-locale»).** Il supporto
  ha confermato che i nomi dipendono dalla lingua della fonte **e** dall'**exchange Betfair** (BF fa
  piccole differenze tra i nomi dell'exchange IT e UK, e usa **ID diversi** tra exchange); è
  un'incoerenza di Betfair, «da verificare puntualmente». Poiché in questo bridge il dizionario è
  **user-built** (la sync Betfair è rimossa) e l'arricchimento ID live è **staccato**, la
  disuniformità per-exchange si gestisce **senza un asse dedicato**: inserisci nel dizionario
  nomi/mercati i **nomi esatti** della tua fonte/exchange e **taggali con la lingua**, così il filtro
  lingua-fonte sceglie le voci giuste. Con questa slice è **completo il meccanismo per-lingua**
  (source_language + dizionari nomi/mercati per-lingua, parità live/anteprima, fail-closed sui typo).
  **NB:** l'epica multilingua **#3 non è ancora chiusa** — resta aperta la slice 4 (localizzazione UI
  completa: banner/log dell'app ancora in italiano).
