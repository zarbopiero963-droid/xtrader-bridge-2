...
| `_process` | `SIGNAL_RECEIVED` → `SIGNAL_PARSED` → `SIGNAL_VALIDATED` → `CSV_WRITTEN` (un segnale scartato si ferma a `SIGNAL_PARSED`, `placeable=false`). Fra i motivi di scarto registrati nel campo `status` c'è **`TEAM_SEPARATOR_NOT_FOUND`** (#182 PR S): separatore squadre impostato nel parser ma non presente nel nome evento. Non richiede cablaggio dedicato — è uno `status` come gli altri e viaggia sul percorso esistente — ma è **nuovo dalla #182**, quindi un diario che non l'ha mai visto non è indicativo |
