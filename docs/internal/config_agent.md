...
| **Redazione segreti** su OGNI contenuto che torna al modello — risultati **e messaggi di rifiuto** | `dispatch` via `ToolRegistry._safe_out` → `event_log.redact_secrets` |
| **Redazione chat ID centralizzata** (audit #137) su OGNI contenuto uscente, non più solo nei tool che se la ricordano: un tool futuro che dimentica `_redact_config` **non può** esporre un ID. Sostituisce solo gli ID **realmente configurati** (mai «i numeri che sembrano un id»), raccolti dalla stessa fonte del report diagnostico. Fail-safe **con memoria** (#171): se la config è momentaneamente illeggibile — tipicamente la finestra di `os.replace` di una scrittura config concorrente — si ripiega sull'**ultimo elenco letto con successo**, non su «nessuna redazione»: degradare a zero aprirebbe una finestra di leak proprio mentre la config viene riscritta. Solo se non c'è **mai** stata una lettura riuscita si resta ai soli segreti. È comunque la **seconda** rete: la prima (`_redact_config`) resta in piedi. **Ripiego fail-closed** (#171): se `redact_chat_ids` stessa solleva, gli ID vengono sostituiti col segnaposto distinto **`<chat-id>`** (`MASKED_CHAT_FALLBACK`) — sostituzione *letterale*, senza confini numerici, quindi può **over-redigere**: in un percorso già degradato è il verso sicuro, e il segnaposto visibile distingue subito una redazione degradata da una regolare. Il provider di default è **`readonly_config_loader`** (fonte unica condivisa col controller): usare il loader operativo riallineerebbe la lingua-CSV e scriverebbe `.bak`, cioè le due mutazioni che la #139 aveva chiuso | `ToolRegistry._safe_out` → `diagnostics.redact_chat_ids`, provider wirato in `build_default_registry` via `diagnostics.collect_chat_ids` |
...
| `get_config_state` maschera token/API key/chat ID — incluse le **chiavi** dei dict `parser_by_chat`/`parser_list_by_chat` (P2-6 audit #76: `{chat_id: parser}` non parte mai in chiaro verso l'API né finisce in cronologia; i nomi parser restano leggibili). **Non è ridondante** con la riga sopra: qui la redazione è *strutturata* sul JSON di config (copre anche le chiavi dei dict), là è la rete di sicurezza sul testo uscente | `_redact_config` |
