...
- **Tool sola-lettura**: `get_config_state` (config **redatta**), `get_health` (semafori
  ...
  **`lookup_dictionary`** (**PR-9 Blocco C**: cerca squadre/mercati/mapping nel dizionario XTrader e
  ...
  I tool sola-lettura caricano la config con `config_store.load_config(sync_csv_language=False)`
  (loader di default del controller, `_readonly_config_loader`): leggere la config per l'assistente
  **non** deve mutare alcuno stato operativo — in particolare **non** riallinea la lingua-CSV
  globale del writer (separatore decimale), a differenza del percorso app normale (audit #137).
