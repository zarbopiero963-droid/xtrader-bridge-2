...
| **Redazione chat ID centralizzata** (audit #137) su OGNI contenuto uscente, non più solo nei tool che se la ricordano: un tool futuro che dimentica `_redact_config` **non può** esporre un ID. Sostituisce solo gli ID **realmente configurati** (mai «i numeri che sembrano un id»), raccolti dalla stessa fonte del report diagnostico. Fail-safe dichiarato: se la config è illeggibile si degrada alla sola redazione dei segreti — è la **seconda** rete, la prima (`_redact_config`) resta in piedi | `ToolRegistry._safe_out` → `diagnostics.redact_chat_ids`, provider wirato in `build_default_registry` via `diagnostics.collect_chat_ids` |
