...
_ALLOWLIST = {
    ...
    "dizionario.py": (1, "gate #311-2.3 `is_validated` fail-safe: dizionario assente/header rotto → "
                         ...
                         "dati inaffidabili"),
    "custom_parser_gui.py": (19, "GUI Tk del costruttore parser: render/azioni best-effort (#182 PR A, 4 nuovi: `_parser_usage` config illeggibile = nessun marcatore ma elenco usabile; `_autoload_active_parser` parser attivo corrotto = editor vuoto, non un pannello che non si apre; `_open_provider_registry` hub irraggiungibile = fallback all'aggiunta rapida; `_highlight_saved` widget distrutto durante un refresh; #182 PR A ⑥, 3 nuovi: `_riga_ha_valore_fisso` widget assente = nessun gate, `_gate_fixed_value` e `_mostra_avviso_valore_fisso` widget distrutto durante un refresh) (P3-28 #76: _builder_snapshot fail-safe — stato non fotografabile = modificato, meglio una conferma in più che una perdita silenziosa); "
