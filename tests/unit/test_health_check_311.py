...
def test_evaluate_operativo_tutto_verde():
    d = _by_key(hc.evaluate(
        ...
        last_signal="Inter v Milan @1.85", csv_ok=True, csv_detail="ok",
...
def test_csv_writable_file_esistente_e_da_creare(tmp_path):
    ...
    ok, motivo = hc.csv_writable(str(p))
    assert ok and "verrà creato" in motivo               # cartella scrivibile
    p.write_text("x", encoding="utf-8")
    ok, motivo = hc.csv_writable(str(p))
    assert ok and "scrivibile" in motivo
    # NON invasiva: il contenuto non è stato toccato dalla sonda.
    assert p.read_text(encoding="utf-8") == "x"


def test_csv_writable_casi_rossi(tmp_path):
    assert hc.csv_writable("") == (False, "csv_path non configurato")
    assert hc.csv_writable(None)[0] is False
    ok, motivo = hc.csv_writable(str(tmp_path))          # è una cartella
    assert not ok and "cartella, non un file" in motivo
    ok, motivo = hc.csv_writable(str(tmp_path / "no" / "segnali.csv"))
    assert not ok and "inesistente" in motivo
...
def test_csv_writable_file_non_scrivibile(tmp_path):
    ...
    try:
        ok, motivo = hc.csv_writable(str(p))
        assert not ok and "NON scrivibile" in motivo
    finally:
