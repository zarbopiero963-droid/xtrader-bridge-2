...
def test_write_quando_reale_e_nuovo():
    ...
    assert lg.decide_write(_real_cfg(), _tracker(), d, "Inter v Milan", now=1000) == lg.WRITE


def test_dry_run_sopprime_la_scrittura():
    d = safety_guard.DailyLimiter(max_per_day=10)
    # dry_run attivo (anche di default se il campo manca) → DRY_RUN, non WRITE
    assert lg.decide_write({"dry_run": True}, _tracker(), d, "X", now=1000) == lg.DRY_RUN
    assert lg.decide_write({}, _tracker(), d, "X", now=1000) == lg.DRY_RUN


def test_duplicato_non_scrive():
    t = _tracker()
    d = safety_guard.DailyLimiter(max_per_day=10)
    assert lg.decide_write(_real_cfg(), t, d, "stesso", now=1000) == lg.WRITE
    # stesso messaggio nella finestra → DUPLICATE (nessuna doppia scommessa)
    assert lg.decide_write(_real_cfg(), t, d, "stesso", now=1001) == lg.DUPLICATE


def test_rate_limited_al_minuto():
    t = _tracker(max_per_minute=2)
    d = safety_guard.DailyLimiter(max_per_day=100)
    assert lg.decide_write(_real_cfg(), t, d, "a", now=1000) == lg.WRITE
    assert lg.decide_write(_real_cfg(), t, d, "b", now=1000) == lg.WRITE
    assert lg.decide_write(_real_cfg(), t, d, "c", now=1000) == lg.RATE_LIMITED
...
def test_limite_giornaliero():
    ...
    assert lg.decide_write(_real_cfg(), t, d, "a", now=1000) == lg.WRITE
    assert lg.decide_write(_real_cfg(), t, d, "b", now=1001) == lg.DAILY_LIMITED
...
def test_duplicato_ha_precedenza_su_dry_run():
    t = _tracker()
    d = safety_guard.DailyLimiter(max_per_day=10)
    lg.decide_write({"dry_run": True}, t, d, "m", now=1000)        # primo: DRY_RUN (registrato)
    assert lg.decide_write({"dry_run": True}, t, d, "m", now=1001) == lg.DUPLICATE


def test_daily_none_nessun_limite_giorno():
    t = _tracker()
    # daily=None → nessun limite giornaliero; resta WRITE
    assert lg.decide_write(_real_cfg(), t, None, "x", now=1000) == lg.WRITE
...
def test_daily_none_con_dry_run_resta_dry_run():
    ...
    assert lg.decide_write({"dry_run": True}, _tracker(), None, "x", now=1000) == lg.DRY_RUN
    assert lg.decide_write(_real_cfg(), _tracker(), None, "y", now=1000) == lg.WRITE
...
def test_rollback_dopo_write_fallita_consente_il_retry():
    ...
    assert lg.decide_write(_real_cfg(), t, d, "sig", now=1000) == lg.WRITE   # consuma
    ...
    assert lg.decide_write(_real_cfg(), t, d, "sig", now=1001) == lg.WRITE
...
def test_duplicato_non_consuma_la_slot_giornaliera():
    t = _tracker()
    d = safety_guard.DailyLimiter(max_per_day=1)
    assert lg.decide_write(_real_cfg(), t, d, "a", now=1000) == lg.WRITE       # consuma 1/1
    # un duplicato successivo è respinto PRIMA del daily: non aggiunge consumo
    assert lg.decide_write(_real_cfg(), t, d, "a", now=1001) == lg.DUPLICATE
    # un messaggio NUOVO ora trova il tetto giornaliero pieno
    assert lg.decide_write(_real_cfg(), t, d, "b", now=1002) == lg.DAILY_LIMITED
...
def test_dry_run_ha_precedenza_su_daily_limited():
    ...
    d = safety_guard.DailyLimiter(max_per_day=1)
    assert lg.decide_write(_real_cfg(), _tracker(), d, "reale", now=1000) == lg.WRITE   # riempie il tetto
    # tetto pieno + simulazione (tracker fresco per non incrociare il dedup) → DRY_RUN, non DAILY_LIMITED
    assert lg.decide_write({"dry_run": True}, _tracker(), d, "sim", now=1001) == lg.DRY_RUN
