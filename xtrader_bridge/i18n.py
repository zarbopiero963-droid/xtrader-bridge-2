...
_CATALOG = {
    "EN": {
        ...
        "❌ Salvataggio config FALLITO: avvio annullato (al riavvio la pulizia del CSV userebbe la config vecchia su disco). Controlla permessi/spazio del file config e riprova.": "❌ Config save FAILED: start cancelled (on restart the CSV cleanup would use the old config on disk). Check permissions/space of the config file and try again.",
    ...
    "ES": {
        ...
        "❌ Salvataggio config FALLITO: avvio annullato (al riavvio la pulizia del CSV userebbe la config vecchia su disco). Controlla permessi/spazio del file config e riprova.": "❌ Guardado de config FALLIDO: inicio cancelado (al reiniciar la limpieza del CSV usaría la config antigua en disco). Comprueba permisos/espacio del archivo config y reinténtalo.",
